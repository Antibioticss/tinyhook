...
static void *trie_query(const uint8_t *export, uint64_t node_off, const char *name, int idx);
...
void *symexp_solve(uint32_t image_index, const char *symbol_name) {
    ...
    symbol_address = trie_query(export_offset, 0, symbol_name, 0);
...
static void *trie_query(const uint8_t *export, uint64_t node_off, const char *name, int idx) {
    ...
    for (int i = 0; i < child_count; i++) {
        ...
        if (memcmp(name + idx, cur_str, cur_len) == 0) {
            return trie_query(export, next_off, name, idx + cur_len);
        }
    }
    return NULL;
}
