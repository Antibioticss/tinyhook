...
int write_mem(void *destnation, const void *source, size_t len) {
	int kr = 0;
	kr |= mach_vm_protect(mach_task_self(), (mach_vm_address_t)destnation, len, FALSE, VM_PROT_READ|VM_PROT_WRITE|VM_PROT_COPY);
	kr |= mach_vm_write(mach_task_self(), (mach_vm_address_t)destnation, (vm_offset_t)source, len);
	kr |= mach_vm_protect(mach_task_self(), (mach_vm_address_t)destnation, len, FALSE, VM_PROT_READ|VM_PROT_EXECUTE);
#ifdef debug
	assert(kr == 0);
#endif
	return kr;
